...
void createwindow(Size windowsize, char* title){
  ...
  SDL_Window *w =
      SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED,
                       SDL_WINDOWPOS_CENTERED, windowsize.width, windowsize.height, 0);
  if (w == NULL) {
...
void renderbox(Point topleft, Point downright, Color color){
  boxRGBA(renderer, topleft.x, topleft.y, downright.x, downright.y, color.R,color.G,color.B,color.A);
}
